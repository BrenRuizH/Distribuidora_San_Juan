...
calcularSumatoria() {
        ...
        if (result.isConfirmed) {
          ...
          const extra = this.remisionEditada.extra;
            if (extra !== undefined && extra !== null && extra !== '') {
                ...
                if (!isNaN(extraNumber)) {
                    formData.append('extra', extraNumber.toFixed(2));
                } else {
                    console.error('Extra no es un número válido:', extra);
                }
            }
            if (this.remisionEditada.extra && this.remisionEditada.descripcion) {
              formData.append('extra', this.remisionEditada.extra);
  ...
  remisionar(id: any) {
    ...
    this.remisionesService.imprimirRemision(id).subscribe((resp: any) => {
      ...
      this.subtotal = this.ordenCompraInfo.reduce((acc: number, orden: { precio: any; total_pares: number; }) => acc + (Number(orden.precio) || 0) * orden.total_pares, 0);
  ...
  <table class="orden-compra-table">
    ...
    <tbody>
      ${this.ordenCompraInfo.map((orden: { total_pares: number; horma: { nombre: any; }; oc: any; detalles: any[];  precio: any; }) => `
        <tr>
          ...
          <td>$${(Number(orden.precio) || 0).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</td>
          <td>$${((Number(orden.precio) || 0) * orden.total_pares).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</td>
        </tr>
      `).join('')}
      <tr>
